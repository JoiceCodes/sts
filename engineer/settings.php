...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($gmailAppPassword)) {
        ...
        if (!$connection) {
            ...
            $stmt = $connection->prepare("INSERT INTO gmail_app_password (user_id, app_password) VALUES (?, ?)");
            $stmt->bind_param("ss", $user_id,$gmailAppPassword); // "s" indicates a string
    ...
    if ($connection) {
        $result = $connection->query("SELECT app_password FROM gmail_app_password WHERE user_id = $user_id");
